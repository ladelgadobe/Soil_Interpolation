...
WORK_DIR <- ".../projectdir/" # <- edit if needed
...
```{r,poly}
poly <- sf::st_read(".../Contourn_area.shp")
...
covariates <- list.files(
  path = ".../Covariates_folder",
  pattern = "\\.tif(f)?$",
  full.names = TRUE)

# Use elevation raster as template if available, otherwise the first raster
elev_idx <- grep("Raster1", basename(covariates), ignore.case = TRUE)
...
names(standardized) <- c("Cov1","Cov2","Cov3","Cov4","Covn")
plot(standardized)
...
g = gstat(formula = Z ~ Cov1+Cov2+Cov3+Cov4+Covn, data=dados) 
...
```{r}
# Spherical
xvalid.sph <- gstat::krige.cv(Z ~ Cov1+Cov2+Cov3+Cov4+Covn, locations = dados, model = fit.sph)
...
# Exponential 
xvalid.exp <- gstat::krige.cv(Z ~ Cov1+Cov2+Cov3+Cov4+Covn, locations = dados, model = fit.exp)
...
# Gaussian
xvalid.gau <- gstat::krige.cv(Z ~ Cov1+Cov2+Cov3+Cov4+Covn, locations = dados, model = fit.gauss)
...
mapa = krige(Z ~ Cov1+Cov2+Cov3+Cov4+Covn, locations = dados, newdata=grid,model = fit.sph)
plot(mapa)
```
