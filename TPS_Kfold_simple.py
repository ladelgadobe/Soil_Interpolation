...
# Función para interpolación TPS con parámetro epsilon
...
# Cargar shapefile y definir grilla
...
# Validación cruzada K-Fold (20 particiones)
y_true = []
y_pred = []
...
for train_index, test_index in kf.split(x):
    ...
    y_true.extend(z_test.tolist())
    y_pred.extend(z_pred.tolist())

rmse = np.sqrt(mean_squared_error(y_true, y_pred))
r2 = r2_score(y_true, y_pred)
lccc = concordance_correlation_coefficient(y_true, y_pred)
print("\nK-Fold CV Evaluation (20 particiones):")
print(f"RMSE: {rmse:.5f}")
print(f"R²: {r2:.5f}")
print(f"LCCC: {lccc:.5f}")

# Interpolación sobre los puntos dentro del polígono
